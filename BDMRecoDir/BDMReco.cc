...
CounterMap_t *InitTreeGen(
    ...
    auto &DaughterStartPoint,
    auto &DaughterStartDirection,
    auto &SwrEnergy,
    auto &TotalMomEvent,
    auto &SunDirectionFromTrueBDM,
    auto &Swrbest_plane)
{
                   ...
                   auto &Swrbest_plane)
{
    ...
    Mode.resize(0);
    InteractionType.resize(0);
    DaughterStartPoint.resize(0);
    DaughterStartDirection.resize(0);
    SwrEnergy.resize(0);
    TotalMomEvent.SetXYZ(0, 0, 0);
    SunDirectionFromTrueBDM.resize(0);
    Swrbest_plane.resize(0);
}
...
int main(int argc, char **argv)
{
    ...
    std::vector<int> PFParticleID, VtxStatus, VtxID, PFP_PdgCode, PFP_Parent, intCCNC, nDaughters, Mode, InteractionType, Swrbest_plane;
    ...
    std::vector<TVector3> DaughterStartPoint, DaughterStartDirection, SunDirectionFromTrueBDM;
    std::vector<std::vector<double>> SwrEnergy;
    TVector3 DaughterStartPoint_tmp, DaughterStartDirection_tmp, TotalMomEvent;
    int n_Events = 0;
    ...
    InitTreeGen(fTree,
                ...
                InteractionType,
                DaughterStartPoint,
                DaughterStartDirection,
                SwrEnergy,
                TotalMomEvent,
                SunDirectionFromTrueBDM,
                Swrbest_plane);
    ...
    for (gallery::Event ev(filenames); !ev.atEnd(); ev.next())
    {
        ...
        for (size_t i = 0; i < MCTruthObjs.size(); i++)
        {
            ...
            Mode.push_back(thisNu.Mode());
            InteractionType.push_back(thisNu.InteractionType());
            ...
            for (int iParticle = 0; iParticle < nParticles; ++iParticle)
            {
                ...
                if (pdgCode == OUT_DM_CODE && MCParticleObjDMIn.StatusCode() == 0)
                {
                    ...
                    SunDirectionFromTrueBDM.push_back(DMMomentum.Vect().Unit()); // SAVE SUN DIRECTION !!!
                }
            }
        }
        ...
        for (size_t iPfp = 0; iPfp < PandoraPfparticleObjs.size(); iPfp++)
        {
            ...
            if (!Pfp_trk.empty())
            {
                for (size_t i = 0; i < Pfp_trk.size(); i++)
                {
                    ...
                    DaughterStartPoint_tmp.SetXYZ(thisTrkPfp->Start().X(), thisTrkPfp->Start().Y(), thisTrkPfp->Start().Z());
                    DaughterStartPoint.push_back(DaughterStartPoint_tmp);
                    DaughterStartDirection_tmp.SetXYZ(thisTrkPfp->StartDirection().X(), thisTrkPfp->StartDirection().Y(), thisTrkPfp->StartDirection().Z());
                    DaughterStartDirection.push_back(DaughterStartDirection_tmp);
                }
            }
        }
        ...
        for (size_t iPfp = 0; iPfp < PandoraPfparticleObjs.size(); iPfp++)
        {
            ...
            if (!Pfp_swr.empty())
            {
                for (size_t i = 0; i < Pfp_swr.size(); i++)
                {
                    ...
                    Swrbest_plane.push_back(thisSwrPfp->best_plane());
                   // std::cout << "thisSwrPfp->best_plane() = " << thisSwrPfp->best_plane() << std::endl;
                    SwrEnergy.push_back(thisSwrPfp->Energy());
